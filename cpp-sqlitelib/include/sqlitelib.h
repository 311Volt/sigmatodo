...
inline void fail(int rc, sqlite3* db = nullptr)
{
...
inline void verify(int rc, sqlite3* db = nullptr, int expected = SQLITE_OK) 
{
	if (rc != expected) {
		fail(rc, db);
	}
...
template <>
inline std::vector<char> get_column_value<std::vector<char>>(sqlite3_stmt *stmt, int col) 
{
...
void bind_value(sqlite3_stmt *stmt, int col, Arg val, sqlite3* db = nullptr) {}
...
template <>
inline void bind_value<double>(sqlite3_stmt *stmt, int col, double val, sqlite3* db) 
{
	verify(sqlite3_bind_double(stmt, col, val), db);
}

template <>
inline void bind_value<std::string>(sqlite3_stmt *stmt, int col, std::string val, sqlite3* db) 
{
	verify(sqlite3_bind_text(stmt, col, val.data(), static_cast<int>(val.size()), SQLITE_TRANSIENT), db);
}

template <>
inline void bind_value<const char *>(sqlite3_stmt *stmt, int col, const char *val, sqlite3* db) {
	verify(sqlite3_bind_text(stmt, col, val, static_cast<int>(strlen(val)), SQLITE_TRANSIENT), db);
}

template <>
inline void bind_value<std::vector<char>>(sqlite3_stmt *stmt, int col, std::vector<char> val, sqlite3* db) {
	verify(sqlite3_bind_blob(stmt, col, val.data(), static_cast<int>(val.size()), SQLITE_TRANSIENT), db);
}

template <bool isRestEmpty, typename T, typename... Rest> struct ValueType;
...
template <typename T, typename... Rest> class Cursor {
public:
	...
	{ 
		return Iterator<T, Rest...>(stmt_.get()); 
	}
...
public:
	Statement(sqlite3 *db, const char *query)
			: db_(db), stmt_(new_sqlite3_stmt(db, query), sqlite3_stmt_deleter) {}
	...
	{
		verify(sqlite3_reset(stmt_.get()), db_);
		bind_values(1, args...);
		return *this;
	}
	...
	{
		bind(args...);
		verify(sqlite3_step(stmt_.get()), db_, SQLITE_DONE);
	}
...
private:
	...
	sqlite3_stmt *new_sqlite3_stmt(sqlite3 *db, const char *query) 
	{
		...
		verify(sqlite3_prepare(db, query, static_cast<int>(strlen(query)), &p, nullptr), db);
		return p;
	}
